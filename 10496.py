...
def solve(beepers, i, visited, left, ini, memo):
    if(left == 0): return abs(ini[0]-beepers[i][0]) + abs(beepers[i][1]-ini[1])
    else:
        if(memo[i][visited] == -1):
            ...
            for j in range(len(beepers)):
                if(not (visited & (1<<len(beepers)-j-1))):
                    beeper = beepers[j]
                    leftDist = min(leftDist, abs(beeper[0]-actual[0])+abs(beeper[1]-actual[1]) + solve(beepers, j, visited ^ (1<<len(beepers)-j-1), left-1, ini, memo))
            memo[i][visited] = leftDist
...
def main():
    ...
    for case in range(cases):
        ...
        print("The shortest path has length " + str(solve(beepers, 0, visited, beepersCnt, ini, memo)))
main()